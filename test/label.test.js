...
test("creates a correctly sized Frontrunner label", () => {
  const pdf = buildLabelPdf({ id: "rec123", createdTime: "2026-08-07T10:00:00Z", fields: { "Job orders": "J-42", "Dropbox Location": "IN-STORE", "BARCODE": "ABC-123" } });
  const text = pdf.toString("ascii");
  assert.match(text, /^%PDF-1\.4/);
  assert.match(text, /\/MediaBox \[0 0 175\.75 283\.46\]/);
  assert.match(text, /IN-STORE/);
  assert.match(text, /FRONTRUNNER/);
  assert.match(text, /BARCODE  ABC-123/);
});
